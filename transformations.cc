...
int transformations() {
	...
	GLFWwindow* window {glfwCreateWindow(1440, 1080, "Transformations", nullptr, nullptr)};
	if (!window)
		return -1;
	...
	unsigned int transformLoc;
	{
		...
		glUseProgram(0);
		transformLoc = static_cast<unsigned int>(
	...
	while (!glfwWindowShouldClose(window)) {
...

		{
			...
			transform = glm::rotate(transform, static_cast<float>(glfwGetTime()), glm::vec3(0.0, 0.0, 1.0f));
			glUniformMatrix4fv(transformLoc, 1, GL_FALSE, glm::value_ptr(transform));
			glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);
		}

		{
			auto const scale {std::sin(glfwGetTime())};
			glm::mat4 transform {1.0f};
			transform = glm::translate(transform, glm::vec3(-0.5f, 0.5f, 0.0f));
